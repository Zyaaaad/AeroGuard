...
double lat_distance_for_tile(int zoom, double latitude)
{
    double scale = cos(latitude * M_PI / 180.0);
    return (360.0 / (1 << zoom)) * scale;
}
...
{
    double tile_size_lon = tile_size_degrees(zoom_level);
    double tile_size_lat = lat_distance_for_tile(zoom_level, center_lat);

    // Ajustement du centre
    area->min_lat = center_lat - (num_drones_side / 2.0) * tile_size_lat;
    area->max_lat = center_lat + (num_drones_side / 2.0) * tile_size_lat;
    area->min_lon = center_lon - (num_drones_side / 2.0) * tile_size_lon;
    area->max_lon = center_lon + (num_drones_side / 2.0) * tile_size_lon;
    ...
    for (int i = 0; i < num_drones_side; i++)
    {
        ...
        {
            float x = area->min_lon + j * tile_size_lon; // Longitude
            float y = area->max_lat - i * tile_size_lat; // Latitude (inversée)
            Drone d;
...
int main()
{
    ...
    int num_drones_side = 5; // Grille 2x2
